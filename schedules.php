		...
		if($_POST['update'] == "0"){
			...
			if(!empty($_POST['sch_name']) and !empty($_POST['sch_desc'])){
				...
				if($_POST['recp_type'] == "s"){
					...
					mysql_query("insert into schedules(sch_id, sch_name, sch_desc, sch_startdate, sch_dates, sch_rec, sch_recp_type, sch_msg_type, sch_recipient, sch_msg, sch_type, sch_enable) values(null, '".$_POST['sch_name']."', '".$_POST['sch_desc']."', '".$_POST['sch_startdate']."', '".$rtimings."', 'r', '".$_POST['recp_type']."', '".$_POST['msg_type']."', '".$recp."', '".$msg."', '".$_POST['sch_type']."', '".$_POST['sch_enable']."');") or die(mysql_error());
			...
			if(!empty($_POST['sch_name']) and !empty($_POST['sch_desc'])){
				...
				if($_POST['recp_type'] == "s"){
					...
					mysql_query("update schedules set sch_name = '".$_POST['sch_name']."', sch_desc = '".$_POST['sch_desc']."', sch_startdate = '".$_POST['sch_startdate']."', sch_dates = '".$rtimings."', sch_recp_type = '".$_POST['recp_type']."', sch_msg_type = '".$_POST['msg_type']."', sch_recipient = '".$recp."', sch_msg = '".$msg."', sch_type = '".$_POST['sch_type']."', sch_enable = '".$_POST['sch_enable']."' where sch_id = '".$_POST['update']."' and sch_rec = 'r';") or die(mysql_error());
					
					echo("<script type=\"text/javascript\">alert('Schedule Updated Successful!'); location.replace('schedules.php?action=mod&uid=".$uid."');</script>");
...
if($_GET['action'] == 'del'){
	...
	mysql_query("delete from schedules where sch_id = ".$_GET['uid']." and sch_rec ='r';") or die(mysql_error());
...
?>
    <div class="box" id="right-panel">
		...
		if(isset($upd) and $upd == 0){
			echo "Add Recurrent Schedule";
		}else{
			echo "Update Recurrent Schedule";	
		}
		?>
        </div>
        <div style="margin:auto; width:780px; height:auto;">
        	<form method="post" action="schedules.php?action=mod">
            <table  width="500px">
                ...
                <tr><td style="text-align:right">Description</td><td><input type="text" name="sch_desc" value="<?php echo $qr['sch_desc']; ?>"></td></tr>
                    ...
                    <tr> <td style="text-align:right">StartDate</td><td><input type="text" name="sch_startdate" value="<?php echo $qr['sch_startdate']; ?>" placeholder="2012-1-1"></td></tr>
                    
                    <tr ><td style="text-align:right">Recurrence Type</td><td>
                    	<?php
                        ...
                        <select name="sch_type">
                            <option value="d" <?php echo chkselect("d", $qr['sch_type']); ?>>Daily</option>
                            <option value="w" <?php echo chkselect("w", $qr['sch_type']); ?>>Weekly</option>
                            <option value="m" <?php echo chkselect("m", $qr['sch_type']); ?>>Monthly</option>
                            <option value="y" <?php echo chkselect("y", $qr['sch_type']); ?>>Yearly</option>
                        </select> 
                    </td></tr>
                    <tr ><td colspan="2" style="text-align:center">Recurrence Timing</td></tr>
                     <tr ><td style="text-align:right">   Hour:</td><td>
                     
                     <?php
					 	if(isset($qr['sch_dates'])){
							
							$timings = split(":", $qr["sch_dates"]);
							
						}
					 ?>
                        <input type="text" name="hr" value="<?php echo $timings[0]; ?>" placeholder="0 - 23"> 0 for Midnight</td></tr>
                    <tr ><td style="text-align:right">    Day:</td><td>
                        <input type="text" name="day" value="<?php echo $timings[1]; ?>" placeholder="1 - 31"></td></tr>
                    <tr ><td style="text-align:right">    WeekDay:</td><td>
                        <input type="text" name="wk" value="<?php echo $timings[2]; ?>" placeholder="0 - 6"> 0 for Sunday</td></tr>
                    <tr ><td style="text-align:right">    Month:</td><td>
                        <input type="text" name="mnth" value="<?php echo $timings[3]; ?>" placeholder="1 - 12"></td></tr>
                    ...
                    <tr><td style="text-align:right">Message</td><td>
                        ...
                        <input type="text" id="smssingle" style="display:none" name="smssingle">
